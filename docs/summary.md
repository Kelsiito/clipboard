...
- History and Library files are encrypted locally with AES-GCM using a device-only Keychain-managed key; private views can optionally require Touch ID or the Mac password.
...
- History and Library data are stored locally in authenticated encrypted envelopes under `~/Library/Application Support/clipboard/`.
- The random 256-bit key is stored in Keychain with device-only, when-unlocked accessibility and is never synchronized or uploaded.
- Plaintext files migrate only after successful decoding; unreadable encrypted files are preserved and disk writes are blocked for that store during the session.
...
- `SecureLocalStorage.swift`: AES-GCM envelopes, Keychain key lifecycle, and macOS device-owner authentication.
...
- Encrypted-storage migration and relaunch with synthetic fixtures; optional device authentication accepted and cancelled.
